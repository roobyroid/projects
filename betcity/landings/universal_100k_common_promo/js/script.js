...
function action(action, data, el) {
	...
	if (/^#.+/i.test(action)) {
		document.documentElement.beforeScrollTop =
...
function openFormCode() {
	...
	formData.classList.add('hide');
	// document.querySelector('.body-wrap').classList.add('heit');
	var t = document.getElementById('phone').value;
	(document.querySelector('.telValue').innerText = ''.concat(t)),
		e.querySelector('.close').addEventListener('click', function () {
			// wrp.classList.remove('no-scroll');
			e.classList.remove('open');
			// document.querySelector('.body-wrap').classList.remove('heit');
		}),
...
function codeValidate() {
	var n = document.querySelectorAll('.code-input'),
		o = !1;
	n.forEach(function (t) {
		t.addEventListener('input', function (e) {
			(o = !(
				...
				1 !== n[2].value.length ||
				1 !== n[3].value.length ||
				'number' != typeof parseInt(t.value)
			)),
				(document.querySelector('.form__submit').disabled = !o);
		});
	});
}
...
if (showFormReg) {
	document.getElementById('phone').addEventListener('focus', function () {
		...
		document.querySelector('.eye').addEventListener('click', function () {
			var cdn =
				document.querySelector('meta[name="img_src"]').content || './img/';
			var e = document.querySelector('#password');
			'password' === e.type
				? ((e.type = 'text'),
				  (document.querySelector('.eye-img').src = cdn + 'icon/icon2.svg'))
				: ((e.type = 'password'),
				  (document.querySelector('.eye-img').src = cdn + 'icon/icon.svg'));
		});
}
...
function startInterval() {
	$('.timer').html(
		'<p>Запросить код повторно можно  через '.concat(sec, ' секунд</p> ')
	),
		(interval = setInterval(function () {
			0 === --sec
				? ($('.sendNewCode__button').show(),
				  $('.timer').html(''),
				  clearInterval(interval),
				  (sec = 60))
				: $('.timer').html(
						'<p>Запросить код повторно можно  через '.concat(
							sec,
							' секунд</p> '
						)
				  );
		}, 1e3)),
		$('.sendNewCode__button').hide();
}
...
$('.sendNewCode__button').on('click', function (e) {
	...
	$('.form__inputs input').keyup(function (e) {
		8 === (e.which || e.keyCode || e.charCode)
			? $(this).prev('input').focus()
			: $(this)
					.val()
					.match(/^\d{1}$/)
			? ($(this).next('input').focus(), $(this).next('input').select())
			: $(this).val('');
	});
var arr = document.querySelectorAll('.code');

function sendCode() {
	var e = document.querySelector('.num1').value,
		t = document.querySelector('.num2').value,
		n = document.querySelector('.num3').value,
		o = document.querySelector('.num4').value;
...
function getTextClipboard(e) {
	...
	for (var n = document.querySelectorAll('.code'), o = 0; o < n.length; o++)
		n[o].value = t[o];
}

$('.form__inputs input').click(function (e) {
	(arr[0].value.length <= 0 &&
	arr[1].value.length <= 0 &&
	arr[2].value.length <= 0 &&
	arr[3].value.length <= 0
		? arr[0]
		: arr[1].value.length <= 0 &&
		  arr[2].value.length <= 0 &&
		  arr[3].value.length <= 0
		? arr[1]
		: arr[2].value.length <= 0 && arr[3].value.length <= 0
		? arr[2]
		: arr[3].value.length <= 0
		? arr[3]
		: e.target
